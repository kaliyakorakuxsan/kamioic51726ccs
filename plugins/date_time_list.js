...
async (conn, mek, m, { reply }) => {
    ...
    let txt = "🌍 *SELECT A COUNTRY NUMBER*\n\nReply with number:\n\n";

    for (let i = 0; i < countryList.length; i++) {
        txt += `*${i + 1}.* ${countryList[i].name}\n`;
...
async (conn, mek, m, { body }) => {
    ...
    if (!m.quoted.text.includes("SELECT A COUNTRY NUMBER")) return;
