...
cmd({
    ...
    react: "🕒",
    desc: "Get time using RapidAPI",
    category: "utility"
}, async (conn, mek, m, { reply, args }) => {
    if (!args[0]) return reply("📌 *Usage:* .timein [location]\n*Example:* .timein Colombo");

    const location = args.join(" ");

    const options = {
        method: 'GET',
        url: 'https://time-date-amp-timezone.p.rapidapi.com/get_time_by_location', // මේක තමයි location එකෙන් ගන්න endpoint එක
        params: { location: location },
        headers: {
            'x-rapidapi-key': '74539283fcmshdcb11a68099cebdp174b35jsn59c9f4c5c4ea',
            'x-rapidapi-host': 'time-date-amp-timezone.p.rapidapi.com'
        }
    };
    ...
    try {
        ...
        if (data) {
            let status = `🕒 *WORLD CLOCK (RAPID API)* 🕒\n\n` +
                         `📍 *Location:* ${location.toUpperCase()}\n` +
                         `📅 *Date:* ${data.date}\n` +
                         `⏰ *Time:* ${data.time}\n` +
                         `🌐 *Timezone:* ${data.timezone}\n\n` +
                         `> *Generated by RANUMITHA-X-MD*`;
            
            reply(status);
        } else {
            reply("❌ රට හෝ නගරය සොයාගත නොහැක.");
        }

    } catch (e) {
        reply("❌ *Error:* API එකේ ගැටලුවක් හෝ Key එක වැරදියි.");
        console.error(e);
    }
});
